...
public class ReplanOptionController {
    ...
    @GetMapping
    @Operation(summary = "재계획 대안 재조회 (PLAN-29)",
            description = "저장된 대안 목록을 반환한다(비교 화면 새로고침). 기준선(KEEP_CURRENT)은 저장 안 하므로 목록에 없다. "
                    + "계획 부재·타인 → 404.")
    public ApiResponse<List<ReplanOptionResponse>> list(
            @CurrentUser UUID userId,
            @PathVariable UUID planId) {
        return ApiResponse.ok(replanService.list(userId, planId));
    }
}
