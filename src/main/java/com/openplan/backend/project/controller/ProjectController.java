...
import com.openplan.backend.project.dto.ProjectListQuery;
...
public class ProjectController {
    ...
    @Operation(summary = "프로젝트 목록 조회 (PROJ-01)",
            description = "status=진행중 그룹(IN_PROGRESS,PAUSED)/종료(CLOSED) 쉼표 다중값. page/size 규약 위반·비정수 → 400. 정렬 createdAt DESC 고정. 조회 시 자동종료 평가 선행.")
    public ApiResponse<List<ProjectResponse>> list(
            @CurrentUser UUID userId,
            @Valid @ModelAttribute ProjectListQuery query) {
        Page<ProjectResponse> result = projectService.list(userId, query.getPage(), query.getSize(), query.getStatus());
