 ...
 * 구글·카카오는 {@code authCode·redirectUri·state}, 네이버는 {@code naverId·appPassword} 를 싣는다.
 * 네이버 캘린더는 오픈 API 에 조회가 없어 CalDAV(Basic)로 붙기 때문이고, 그쪽에는 인가 단계가 아예 없다.
 *
 * <p><b>왜 {@code @NotBlank} 를 떼었는가.</b> 빈 검증은 "이 필드는 항상 필요하다"만 말할 수 있어
 * <b>제공자에 따라 갈리는 필수 조건을 표현하지 못한다.</b> 그대로 두면 네이버 요청이 authCode 없음으로
