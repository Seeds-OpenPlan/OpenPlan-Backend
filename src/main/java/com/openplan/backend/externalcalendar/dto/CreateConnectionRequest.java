 ...
 * 구글·카카오는 {@code authCode·redirectUri·state}, 애플은 {@code appleId·appPassword} 를 싣는다.
 * 애플 캘린더는 오픈 API 에 조회가 없어 CalDAV(Basic)로 붙기 때문이고, 그쪽에는 인가 단계가 아예 없다.
 *
 ...
 *
 * <p>{@code appPassword}는 애플 계정 보안 설정에서 발급하는 <b>2단계 인증 애플리케이션 비밀번호</b>이며 일반
